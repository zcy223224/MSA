from math import exp, sqrt
from random import randint
import numpy as np
from module.grows import lowbest
from module.chaotic import PWLCM
from module.functions_mod import F1,F2,F3,F4,F5,F6,F7,F8,F9,F10,F11,F12,Ackley,Perm,Trid,Zakharov,Powell, midstep, overlimit
from module.levy import fly

'''种族个体数'''
maxnumber=100
'''维度'''
dimension=30
'''发现者比例'''
ratep=0.1
'''loser者比例'''
ratea=0.1
'''函数名列表，各函数最大边界'''
functions=[F1,F2,F3,F4,F5,F6,F7,F8,F9,F10,F11,F12,Ackley,Perm,Trid,Zakharov,Powell]
domainlist=[100,10,100,100,30,100,1.28,500,5.12,32,600,50,32,30,900,10,5]

def fitness(elem):
    return elem[dimension]

def OOZ():
    if(randint(0,1)):
        return 1
    else:
        return -1

def stepfun(targetacc,point1,point2,point3,point4,point5):
    count=0
    line1=[point2-point1,point3-point2,point4-point3,point5-point4]
    line2=[point3-point1,point4-point2,point5-point3]
    line3=[point4-point1,point5-point2]
    line4=point5-point1
    for var in line1:
        if(var<targetacc):
            count+=1
    for var in line2:
        if(var<targetacc):
            count+=2
    for var in line3:
        if(var<targetacc):
            count+=3
    if(line4<targetacc):
        count+=4
    return 0.1*count
'''2,4,6,8,14,15'''
functionorder=14
wholefunc=15
while(functionorder<wholefunc):
    '''最大边界'''
    domain=domainlist[functionorder]

    print('现在测试的函数是:{}'.format(functions[functionorder].__name__))
    '''第iter_table次实验，共5次'''
    iter_table=0
    iter_tablemax=5
    fitnessvalue=[]
    while(iter_table<iter_tablemax):
        '''第iter_table次实验'''

        '''整个种族'''
        specie=[]
        '''个体'''
        Individual=[]
        '''观察点'''
        Observepoint=[0,0,0,0,0]
        '''初始化种群'''
        counter=0
        while(counter<maxnumber):
            start=int(np.random.rand()*10000)+500
            Individual=PWLCM(start,dimension,domain)
            Reverse=[]
            for var in Individual:
                if(var>0):
                    Reverse.append(domain-var)
                else:
                    Reverse.append(-domain-var)
            Reverse.append(0)
            Reverse[dimension]=functions[functionorder](Reverse)
            Individual.append(0)
            Individual[dimension]=functions[functionorder](Individual)
            specie.append(Individual)
            specie.append(Reverse)
            counter+=1

        fromlowbest=lowbest(functionorder,dimension)
        fromlowbest[dimension]=functions[functionorder](fromlowbest)
        specie.append(fromlowbest)
        specie.sort(key=fitness)
        for var in range(maxnumber+1):
            specie.remove(specie[maxnumber])

        '''算法,开始迭代'''       
        '''最大迭代次数'''
        iteration=500
        n=0
        before=[]
        while(n<iteration):
            '''1更新中'''
            i=0
            while(i<int(maxnumber*ratep)):
                j=0
                while(j<dimension):
                    beforevalueofij=specie[i][j]
                    beforevalueoffitness=specie[i][dimension]
                    if(n<5):       
                        specie[i][j]+=np.random.randn()*domain*0.1*exp(-n*0.03)
                    else:
                        specie[i][j]+=np.random.randn()*domain*stepfun(1,Observepoint[0],Observepoint[1],Observepoint[2],Observepoint[3],Observepoint[4])*exp(-n*0.03)
                    specie[i][j]=overlimit(domain,specie[i][j])
                    specie[i][dimension]=functions[functionorder](specie[i])
                    if(beforevalueoffitness<specie[i][dimension]):
                        specie[i][j]=beforevalueofij
                        specie[i][dimension]=beforevalueoffitness
                    j+=1
                i+=1

            specie.sort(key=fitness)

            '''2更新中'''
            while(i<maxnumber):
                j=0
                while(j<dimension):
                    specie[i][j]+=0.1*(specie[0][j]-specie[i][j])
                    specie[i][j]=overlimit(domain,specie[i][j])
                    j+=1
                specie[i][dimension]=functions[functionorder](specie[i])
                i+=1

            specie.sort(key=fitness)

            '''3更新中'''
            i=int(maxnumber*(1-ratea))
            while(i<maxnumber):
                before=[]
                for var in specie[i]:
                    before.append(var)
                j=0
                while(j<dimension):
                    specie[i][j]=0
                    j+=1
                for var1 in range(20):
                    vectorofLevy=fly(domain,dimension)                                        
                    j=0
                    while(j<dimension):
                        specie[i][j]+=vectorofLevy[j]
                        specie[i][j]=overlimit(domain,specie[i][j])
                        j+=1
                    specie[i][dimension]=functions[functionorder](specie[i])
                    if(float(before[dimension])<float(specie[i][dimension])):
                        specie[i]=before
                    else:
                        before=[]
                        for var2 in specie[i]:
                            before.append(var2)
                i+=1
            if(n<5):
                Observepoint[n]=specie[0][dimension]
            n+=1
            specie.sort(key=fitness)

        '''第iter_table次实验结束，更新最优值'''
        iter_table+=1
        fitnessvalue.append(specie[0][dimension])
        print('第{}次计算完成,最优值为:{}'.format(iter_table,specie[0][dimension]))
    '''15次实验结束，生成3个值'''
    average=float(sum(fitnessvalue))/len(fitnessvalue)
    total = 0
    for value in fitnessvalue:
        total += (value - average) ** 2
    
    stddev = sqrt(total/len(fitnessvalue))
    fitnessvalue.sort()
    print('最优值:{}    平均值:{}    标准差:{}'.format(fitnessvalue[0],average,stddev))
    functionorder+=1
    print("\n\n\n\n")